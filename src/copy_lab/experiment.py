...
import torch  # noqa: E402  (env vars above must be set before import)
...
def run(length=32, samples=16, seed=42, device="cpu", output=ROOT / "results"):
    ...
    def evaluate(tokens):
        with torch.inference_mode():
            return torch.cat(
                [
                    model(row[None].to(device), return_type="loss", loss_per_token=True).cpu()
                    for row in tokens
                ]
            )
    ...
    ax.plot(
        positions,
        repeated_loss[:, length + 1 : 2 * length].mean(0),
        label="Repeated context: A → A",
    )
    ax.plot(
        positions,
        control_loss[:, length + 1 : 2 * length].mean(0),
